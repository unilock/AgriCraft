...
public class SoilsDatagen {
	...
	public static void registerSoils(BootstrapContext<AgriSoil> context) {
		...
		mc(context, "sand", AgriSoil.builder().variants(AgriSoilVariant.builder().tag("c:sands").build(), AgriSoilVariant.builder().tag("minecraft:sand").build()).humidity(ARID).acidity(NEUTRAL).nutrients(LOW).growthModifier(0.8).build());
		...
		agricraft(context, "gravel", AgriSoil.builder().variants(AgriSoilVariant.builder().tag("c:gravels").build()).humidity(DRY).acidity(SLIGHTLY_ALKALINE).nutrients(LOW).growthModifier(0.5).build());
	}
